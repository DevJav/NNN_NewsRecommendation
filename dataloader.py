...
class NRMSDataLoader(NewsrecDataLoader):
    ...
    def __getitem__(self, idx) -> tuple[tuple[np.ndarray], np.ndarray]:
        ...
        his_input_time:     (samples, history_size, 1)
        ...
        # print("\nBatch_X:", batch_X)
        # # print("\batch_X[self.history_column].to_list():", batch_X[self.history_column].to_list())
        # # print("\nbatch_X[self.timestamp_column].to_list():", batch_X[self.timestamp_column].to_list())
        ...
        if self.eval_mode:
            ...
            # print("his_input_title:", his_input_title)
            his_input_time = np.array([
                [item.timestamp() for item in sublist] for sublist in batch_X[self.timestamp_column].to_list()
            ])
            his_input_time = np.repeat(his_input_time, repeats=repeats, axis=0)
            # =>
        ...
        # print(f"__getitem__: his_input_title.shape: {his_input_title.shape}")
        # print(f"__getitem__: his_input_time.shape: {his_input_time.shape}")
        # print(f"__getitem__: pred_input_title.shape: {pred_input_title.shape}")
        return (his_input_title, his_input_time, pred_input_title), batch_y
...
class NRMSDataLoaderPretransform(NewsrecDataLoader):
    ...
    def __getitem__(self, idx) -> tuple[tuple[np.ndarray], np.ndarray]:
        ...
        if self.eval_mode:
            ...
            # =>
            his_input_time = np.array(
                batch_X[self.timestamp_column].to_list()
            ).reshape(-1, 1)
            # =>
            pred_input_title = self.lookup_article_matrix[
                batch_X[self.inview_col].explode().to_list()
            ...
            his_input_time = np.array(batch_X[self.timestamp_column].to_list()).reshape(
                -1, 1
            )
            pred_input_title = self.lookup_article_matrix[
                batch_X[self.inview_col].to_list()
